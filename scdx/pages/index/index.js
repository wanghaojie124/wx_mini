...
Page({
    ...
    data: {
        ...
        articleType: ['推荐阅读', '学校通知', '吃喝玩乐', '人物志'], // 资讯搜索
        articleList: [], // 资讯列表
        limit: 10, // 每页显示条数
        pages: 0, // 总页数
    ...
    bannerTo: function(e) {
        ...
        if (appId) {
            ...
            if (path == '/pages/curriculum/curriculum') {
                this.openCurriculum() // 查课表
            } else {
                wx.navigateTo({
    ...
    openClass: function () {
        ...
        if (uid) {
            wx.navigateTo({
                url: '/pages/classReview/classReview'
            })
        } else {
            wx.showToast({
                title: '请先登陆',
    ...
    openArticle: function() {
        wx.navigateTo({
            url: '/pages/webView/webView?title=校历&path=https://mp.weixin.qq.com/s/yhqWlhSKdj5wVcNBQYa8Lw'
        })
    },
    ...
    openOptimization: function() {
        wx.navigateTo({
            url: '/pages/webView/webView?title=空教室&path=https://scuroom.cdut.top/cir/index.html'
        })
    },
